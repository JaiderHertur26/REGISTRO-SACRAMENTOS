...
import { 
    ...
    ShieldCheck, ArrowLeft, Search, Droplet, Hash, AlertCircle
} from 'lucide-react';
...
const ConfirmationCelebratedPage = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        fechaSacramento: '', lugarSacramento: '', 
        apellidos: '', nombres: '', sexo: '', 
        fechaNacimiento: '', edad: '', lugarNacimiento: '', 
        tipoUnionPadres: '', nombrePadre: '', cedulaPadre: '', 
    ...
    useEffect(() => {
        const loadInitialData = async () => {
            ...
            setFormData(prev => ({ 
                ...
                numeroRegistro: p?.numeroRegistroActual || '',
    ...
    // 🚀 MÁQUINA DEL TIEMPO: SINCRONIZA DA FE 
    useEffect(() => {
    ...
    const handleChange = (e) => {
        ...
        const uppercaseFields = ['nombres', 'apellidos', 'lugarNacimiento', 'lugarSacramento', 'lugarBautismo', 'padrinos', 'nombrePadre', 'nombreMadre', 'ministro', 'daFe', 'notaMarginal'];
            ...
            <DashboardLayout entityName={nombreParroquia}>
                <div className="print:hidden max-w-xl mx-auto bg-white p-12 rounded-[3rem] shadow-xl border border-gray-100 text-center mt-12 animate-in fade-in duration-500">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <Button onClick={() => navigate('/parroquia/confirmacion/sentar-registros')} className="py-7 rounded-2xl bg-[#4B7BA7] text-white font-black uppercase text-[10px] shadow-xl shadow-blue-900/20">Sentar Libros</Button>
                    </div>
                </div>
    ...
    const SectionHeader = ({ icon: Icon, title, number }) => (
        <div className="flex items-center gap-3 mb-8 pb-3 border-b border-gray-100 mt-10 first:mt-2">
            <div className="w-8 h-8 rounded-2xl bg-[#D4AF37] text-white flex items-center justify-center text-xs font-black shadow-lg shadow-yellow-900/20">{number}</div>
                ...
                <div className="max-w-5xl mx-auto pb-20 pt-6">
                    ...
                    <div className="mb-10 flex flex-col md:flex-row justify-between items-end gap-6">
                        <div className="flex items-center gap-4">
                            <Button variant="ghost" onClick={() => navigate(-1)} className="rounded-full w-12 h-12 p-0 bg-white border border-gray-200 text-gray-400 hover:text-gray-900 hover:bg-gray-100 shadow-sm transition-all"><ArrowLeft className="w-5 h-5"/></Button>
                            <div>
                        ...
                        <div className="p-12 space-y-10">
                            ...
                            <section>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-4 gap-8 bg-slate-50 p-8 rounded-[2rem] border border-slate-100 shadow-inner">
                                    <div><label className={labelClass}>Nº Inscripción</label><input name="numeroRegistro" value={formData.numeroRegistro} disabled className={`${inputClass} opacity-70 text-red-600 font-mono`} /></div>
                                    <div><label className={labelClass}>Libro</label><input name="Libro" required value={formData.Libro} onChange={handleChange} disabled className={`${inputClass} opacity-70 font-mono text-2xl font-black`} /></div>
                                    <div><label className={labelClass}>Folio</label><input name="folio" required value={formData.folio} onChange={handleChange} disabled className={`${inputClass} opacity-70 font-mono text-2xl font-black`} /></div>
                                    <div><label className={labelClass}>Acta Nº</label><input name="numero" required value={formData.numero} onChange={handleChange} disabled className={`${inputClass} opacity-70 font-mono text-2xl font-black`} /></div>
                            ...
                            <section>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                    <div><label className={labelClass}>Fecha Confirmación</label><input type="datetime-local" name="fechaSacramento" required value={formData.fechaSacramento} onChange={handleChange} className={inputClass} /></div>
                                    <div><label className={labelClass}>Parroquia / Lugar</label><input name="lugarSacramento" required value={formData.lugarSacramento} onChange={handleChange} className={inputClass} /></div>
                                </div>
                            </section>
                            ...
                            <section>
                                <div className="flex justify-between items-center mb-8 border-b border-gray-100 pb-3 mt-10 first:mt-2">
                                    <div className="flex items-center gap-3">
                                        <div className="w-8 h-8 rounded-2xl bg-[#D4AF37] text-white flex items-center justify-center text-xs font-black shadow-lg shadow-yellow-900/20">03</div>
                            ...
                            <section>
                                ...
                                <div>
                                    <label className={labelClass}>Nota Marginal / Observaciones Iniciales</label>
                                    <textarea name="notaMarginal" value={formData.notaMarginal} onChange={handleChange} className={`${inputClass} h-24 resize-none font-mono text-xs`} placeholder="INFORMACIÓN ADICIONAL PARA EL REGISTRO..." />
                                </div>
                            </section>
