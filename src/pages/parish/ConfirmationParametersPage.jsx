...
const ConfirmationParametersPage = () => {
    ...
    const { getConfirmationParameters, updateConfirmationParameters, resetConfirmationParameters } = useAppData(); // 🚀 CEREBRO CENTRAL
    ...
    useEffect(() => {
        const loadSettings = async () => {
            ...
            try {
                const params = await getConfirmationParameters(user.parishId);
                if (params) {
        ...
        loadSettings();
    }, [user, getConfirmationParameters, toast]);
    ...
    const handleSaveParameters = async () => {
        ...
        setIsSaving(true);
        try {
            const res = await updateConfirmationParameters(user.parishId, cloudParams);
    ...
    const handleResetParameters = async () => {
        if (window.confirm('¿Está seguro de que desea reiniciar todos los consecutivos a sus valores de fábrica? Esta acción afectará a todos los ordenadores.')) {
            setIsSaving(true);
            try {
                await resetConfirmationParameters(user.parishId);
                const freshParams = await getConfirmationParameters(user.parishId);
                setCloudParams(freshParams);
                toast({ title: "Parámetros Reiniciados", description: "Se han restaurado los valores por defecto en la nube." });
            } catch (error) {
                toast({ title: "Error", description: "Fallo al restaurar parámetros.", variant: "destructive" });
            } finally {
                setIsSaving(false);
            }
        }
    ...
    return (
        <DashboardLayout entityName={user?.parishName || "Parroquia"}>
            ...
            <div className="flex border-b border-gray-300 mb-6 space-x-1 overflow-x-auto">
                ...
                <button className="px-6 py-2 text-sm font-bold text-red-600 border-b-2 border-red-600 bg-white rounded-t-md whitespace-nowrap">
                    Confirmaciones
                </button>
            ...
            <div className="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden text-sm">
                ...
                <div className="p-4 border-b border-gray-100 bg-gray-50/50">
                    <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                        <CheckSquare className="w-4 h-4 text-red-600" /> Preferencias de este Ordenador
                    </h3>
                ...
                <div className="p-4 border-b border-gray-100">
                    <div className="flex flex-wrap items-center justify-between mb-3">
                        <h3 className="text-sm font-black text-gray-800 uppercase flex items-center gap-2">
                            <BookOpen className="w-4 h-4 text-red-600" /> Consecutivos Libro Ordinario
                        </h3>
                    </div>
                        ...
                        <label className="flex items-center gap-2 text-sm text-gray-700 cursor-pointer select-none">
                         ...
                         <label className="flex items-center gap-2 text-sm text-gray-700 cursor-pointer select-none">
                            <input 
                                ...
                                className="w-4 h-4 text-red-600 border-gray-300 rounded focus:ring-red-600 disabled:opacity-50" 
                            />
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                        <div>
                            ...
                            <input 
                                ...
                                className="w-full px-3 py-2 border border-gray-200 rounded-lg font-bold text-gray-800 focus:ring-2 focus:ring-red-600 outline-none disabled:bg-gray-50"
                            />
                        </div>
                        <div>
                            ...
                            <input 
                                ...
                                className="w-full px-3 py-2 border border-gray-200 rounded-lg font-mono font-bold text-red-600 focus:ring-2 focus:ring-red-600 outline-none disabled:bg-gray-50"
                            />
                        </div>
                        <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-100 border-b border-gray-100 bg-gray-50/30">
                    <div className="p-6">
                        <h4 className="font-bold text-gray-800 mb-3 text-[11px] uppercase tracking-widest">Inscripción Regular (Adulto/Niño) en:</h4>
                        <div className="flex flex-col gap-3">
                            <label className="flex items-center gap-2 cursor-pointer select-none">
                                <input type="radio" name="registroRegularEn" value="ordinario" checked={cloudParams.registroRegularEn === 'ordinario'} onChange={handleCloudChange} className="w-4 h-4 text-red-600 focus:ring-red-600" />
                                <span className="text-gray-700 text-sm font-medium">Libro Ordinario</span>
                            </label>
                            <label className="flex items-center gap-2 cursor-pointer select-none">
                                <input type="radio" name="registroRegularEn" value="suplementario" checked={cloudParams.registroRegularEn === 'suplementario'} onChange={handleCloudChange} className="w-4 h-4 text-red-600 focus:ring-red-600" />
                    ...
                    <div className="p-6">
                        ...
                        <div className="flex flex-col gap-3">
                            <label className="flex items-center gap-2 cursor-pointer select-none">
                                <input type="radio" name="registroDecretoEn" value="ordinario" checked={cloudParams.registroDecretoEn === 'ordinario'} onChange={handleCloudChange} className="w-4 h-4 text-red-600 focus:ring-red-600" />
                                <span className="text-gray-700 text-sm font-medium">Libro Ordinario</span>
                            </label>
                            <label className="flex items-center gap-2 cursor-pointer select-none">
                                <input type="radio" name="registroDecretoEn" value="suplementario" checked={cloudParams.registroDecretoEn === 'suplementario'} onChange={handleCloudChange} className="w-4 h-4 text-red-600 focus:ring-red-600" />
                                <span className="text-gray-700 text-sm font-medium">Libro Supletorio</span>
                            </label>
                        </div>
                    </div>
                ...
                <div className="p-6 border-b border-gray-100">
                    ...
                    <label className="flex items-center gap-2 cursor-pointer select-none">
                        <input type="checkbox" name="generarNotaMarginal" checked={cloudParams.generarNotaMarginal || false} onChange={handleCloudChange} className="w-4 h-4 text-red-600 border-gray-300 rounded focus:ring-red-600" />
                        <span className="text-gray-700 font-medium text-sm">Generar nota marginal automáticamente al imprimir el libro</span>
                    </label>
                </div>

                {/* Footer / Actions */}
                <div className="p-4 bg-gray-50 border-t border-gray-200 flex flex-col md:flex-row justify-end gap-3 sticky bottom-0 z-10">
                    <Button 
                    ...
                    <Button 
                        ...
                        className="bg-red-600 hover:bg-red-700 text-white rounded-xl px-8 font-black uppercase tracking-widest text-[10px] shadow-lg shadow-red-900/20 transition-all active:scale-95"
                    >
