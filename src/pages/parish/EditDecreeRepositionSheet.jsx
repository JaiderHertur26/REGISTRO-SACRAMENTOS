...
const EditDecreeRepositionSheet = () => {
    ...
    useEffect(() => {
        const loadDecreeData = async () => {
            ...
            try {
                ...
                const bd = payload.newPartidaSummary || {};
                setNewPartida({
                    ...payload,
                    ...
                    paternalGrandparents: payload.paternalGrandparents || payload.abuepat || '',
                    maternalGrandparents: payload.maternalGrandparents || payload.abuemat || '',
                    godparents: payload.godparents || payload.padrinos || '',
    ...
    const handleUpdate = async (e) => {
        ...
        try {
            ...
            const notaReposicion = `ESTA PARTIDA SE INSCRIBE POR REPOSICIÓN SEGÚN DECRETO NO. ${decreeData.decreeNumber.toUpperCase()} DE FECHA ${fechaTexto}, DEBIDO A LA ${causaText} DEL ORIGINAL. LA INFORMACIÓN SUMINISTRADA ES FIEL A LA CONTENIDA EN EL LIBRO SUPLETORIO.`;
    ...
    return (
        <DashboardLayout entityName={user?.parishName || "Parroquia"}>
                        ...
                        <form onSubmit={handleUpdate} className="p-10 space-y-12 animate-in fade-in slide-in-from-right-4 duration-500">
                            <section>
                                <div className="flex items-center justify-between mb-8 pb-4 border-b border-gray-50">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <h3 className="text-sm font-black text-gray-900 uppercase tracking-[0.2em]">01. Datos del Decreto</h3>
                                    </div>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <select name="conceptoAnulacionId" value={decreeData.conceptoAnulacionId} onChange={handleDecreeChange} className="w-full h-[50px] px-4 border border-gray-200 rounded-xl outline-none font-bold text-xs bg-gray-50 uppercase tracking-tighter">
                            ...
                            <section>
                                <div className="flex items-center gap-3 mb-8 pb-4 border-b border-gray-50">
                                    ...
                                    <h3 className="text-sm font-black text-gray-800 uppercase tracking-[0.2em]">02. Partida Supletoria</h3>
                                </div>

                                <div className="space-y-8">
                                    <div className="grid grid-cols-3 gap-6 bg-gray-50/50 p-6 rounded-3xl border border-gray-100 shadow-inner">
                                        <div className="space-y-1"><label className="text-[10px] font-black text-gray-400 uppercase ml-1 text-center block">Libro</label><Input name="book_number" value={newPartida.book_number} onChange={handleNewPartidaChange} className="bg-white text-center font-black font-mono text-lg py-6" /></div>
                                        <div className="space-y-1"><label className="text-[10px] font-black text-gray-400 uppercase ml-1 text-center block">Folio</label><Input name="page_number" value={newPartida.page_number} onChange={handleNewPartidaChange} className="bg-white text-center font-black font-mono text-lg py-6" /></div>
                                        <div className="space-y-1"><label className="text-[10px] font-black text-gray-400 uppercase ml-1 text-center block">Acta</label><Input name="entry_number" value={newPartida.entry_number} onChange={handleNewPartidaChange} className="bg-white text-center font-black font-mono text-lg py-6" /></div>
                                    </div>

                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                        <div className="space-y-2"><label className="text-[10px] font-black text-gray-400 uppercase ml-1">Apellidos</label><Input name="lastName" value={newPartida.lastName} onChange={handleNewPartidaChangeUpper} className="py-6 font-black uppercase text-gray-800" /></div>
                                        <div className="space-y-2"><label className="text-[10px] font-black text-gray-400 uppercase ml-1">Nombres</label><Input name="firstName" value={newPartida.firstName} onChange={handleNewPartidaChangeUpper} className="py-6 font-black uppercase text-gray-800" /></div>
                                    </div>

                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                        <div className="space-y-2"><label className="text-[10px] font-black text-gray-400 uppercase">F. Bautismo</label><Input type="date" name="sacramentDate" value={newPartida.sacramentDate} onChange={handleNewPartidaChangeRaw} className="py-6" /></div>
                                        <div className="space-y-2"><label className="text-[10px] font-black text-gray-400 uppercase">F. Nacimiento</label><Input type="date" name="birthDate" value={newPartida.birthDate} onChange={handleNewPartidaChangeRaw} className="py-6" /></div>
                                        <div className="space-y-2">
                                            <label className="text-[10px] font-black text-gray-400 uppercase">Sexo</label>
                                            <select name="sex" value={newPartida.sex} onChange={handleNewPartidaChangeRaw} className="w-full h-[50px] px-4 border border-gray-200 rounded-xl font-bold bg-gray-50 uppercase text-xs">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-8 border-t border-dashed border-gray-100">
                                        <div className="bg-blue-50/50 p-6 rounded-[2.5rem] border border-blue-100/50 space-y-4 shadow-sm">
                                            ...
                                            <Input name="fatherName" value={newPartida.fatherName} onChange={handleNewPartidaChangeUpper} className="bg-white font-bold uppercase text-xs" />
                                            <Input name="paternalGrandparents" value={newPartida.paternalGrandparents} onChange={handleNewPartidaChangeUpper} className="bg-white text-[10px] uppercase" />
                                        </div>
                                        <div className="bg-pink-50/50 p-6 rounded-[2.5rem] border border-pink-100/50 space-y-4 shadow-sm">
                                            ...
                                            <Input name="motherName" value={newPartida.motherName} onChange={handleNewPartidaChangeUpper} className="bg-white font-bold uppercase text-xs" />
                                            <Input name="maternalGrandparents" value={newPartida.maternalGrandparents} onChange={handleNewPartidaChangeUpper} className="bg-white text-[10px] uppercase" />
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-2 pt-4">
                                        ...
                                        <Input name="godparents" value={newPartida.godparents} onChange={handleNewPartidaChangeUpper} className="py-6 uppercase font-bold text-gray-600 shadow-sm" />
                                    </div>

                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 border-t pt-8">
                                        <div className="space-y-2"><label className="text-[10px] font-black text-gray-400 uppercase ml-1">Sacerdote Celebrante</label><Input name="minister" value={newPartida.minister} onChange={handleNewPartidaChangeUpper} className="py-6 uppercase font-black text-blue-900 shadow-sm" /></div>
                                        <div className="space-y-2"><label className="text-[10px] font-black text-gray-400 uppercase ml-1">Firma (Da Fe)</label><Input name="ministerFaith" value={newPartida.ministerFaith} onChange={handleNewPartidaChangeUpper} className="py-6 uppercase font-bold text-gray-500 bg-gray-50" /></div>
                                    </div>
                                </div>
                            </section>

                            <div className="flex justify-end gap-4 border-t border-gray-100 pt-10 sticky bottom-0 bg-white">
                                <Button type="button" variant="outline" onClick={() => setShowDeleteModal(true)} disabled={isLoading} className="text-red-600 border-red-200 hover:bg-red-50"><Trash2 className="w-4 h-4 mr-2" /> Eliminar</Button>
                                <Button type="submit" disabled={isLoading} className="bg-gradient-to-r from-green-600 to-green-700 text-white font-black px-8 uppercase text-xs"><Save className="w-4 h-4 mr-2" /> Guardar Cambios</Button>
                            </div>
                        </form>
                    )}
                </div>
            ...
            <ConfirmationDialog 
                ...
                message="Esta acción borrará el registro del decreto y eliminará la partida supletoria de la Nube. No se puede deshacer."
