...
export const AppDataProvider = ({ children }) => {
  ...
  const saveBaptismToSource = async (data, parishId, mode) => {
      ...
      try {
          ...
          const dbRecord = {
              ...
              tipo_union_padres: String(purificado.tipoUnionPadres || '1'),
  ...
  const updateNotificationStatus = async (notificationId, status) => {
      ...
      Object.keys(parishNotifications).forEach(pId => {
          ...
          if (index !== -1) {
              // 🚀 Mapeo profundo para que React detecte el cambio real
      ...
      if (updated) {
          ...
          window.dispatchEvent(new Event('storage')); // 🚀 ESTO ACTUALIZA EL SIDEBAR AL INSTANTE
          return { success: true };
      }
  ...
  const getBaptismCorrections = (parishId) => {
    ...
    try {
        ...
        // 🚀 OBTENCIÓN SEGURA DE L:F:N ORIGINALES
        ...
        // 🚀 ACTUALIZACIÓN DE PARÁMETROS MATEMÁTICA Y SINCRONIZADA
        ...
        // 2. Guardar en Supabase (Parámetros)
        try {
            ...
            console.warn("Fallo menor al subir parámetros a la nube:", e);
        }
        
        // 3. Guardar en Supabase (Partidas)
        try {
        ...
        } catch(e) { console.warn("Fallo menor al subir partidas a la nube:", e); }
        
        // 🚀 CÁPSULA DEL DECRETO (AQUÍ SE INYECTA LA OBSERVACIÓN PARA QUE LLEGUE AL IMPRESO)
        ...
        const decreeRecord = {
            ...
            type: 'correction',
            sacrament: 'bautismo',
            numeroDecreto: decreeData.decreeNumber, 
            fechaDecreto: decreeData.decreeDate,    
            observaciones: decreeData.observaciones, // 🚀 AÑADIDO: Captura el texto del formulario
            originalPartidaId, 
            ...
            newPartidaSummary: { 
                ...newPartidaRaw, 
                book: partidaNuevaObj.libro, 
                page: partidaNuevaObj.folio, 
                entry: partidaNuevaObj.numero 
            },
            status: 'active',
            createdAt: new Date().toISOString()
        };
        ...
        try {
            ...
            if (changedRecordsForSupabase.length > 0) {
                 ...
                 const dbRecords = changedRecordsForSupabase.map(newItem => ({
                    ...
                    sacrament_date: cleanDate(newItem.sacramentDate || newItem.fechaSacramento || newItem.fechaBautismo || newItem.fecbau),
                    ...
                    tipo_union_padres: String(newItem.tipoUnionPadres || newItem.tipo_union_padres || '1'),
 ...
 const deleteParroco = async (id, parishId) => {
    ...
    const seatBaptism = async (originalId, parishId, updates = {}) => {
      try {
          ...
          const dbRecord = {
              ...
              tipo_union_padres: String(finalRecord.tipoUnionPadres || finalRecord.tipo_union_padres || '1'),
              status: 'seated',
  ...
  const seatConfirmation = async (id, parishId) => {
      ...
      const finalRecord = { 
          ...record, status: 'celebrated', book_number: libroAsignado, page_number: folioAsignado, entry_number: numeroAsignado 
      };
  ...
  const seatMatrimonio = async (id, parishId) => {
      ...
      const nextConsecutivos = calculateNextConsecutive(
          params.ordinarioNumero || 1, params.ordinarioFolio || 1, params.ordinarioLibro || 1, params.ordinarioPartidas || 1, params.ordinarioRestartNumber
      );
  ...
  const createChanceryCorrection = async (decreeData, originalPartidaId, newPartidaData, targetParishId, chanceryId) => {
    try {
      ...
      // 🚀 GENERACIÓN DE NOTAS CON EL MOTOR DE APPDATACONTEXT
      const notaAnulada = generarNotaAlMargenAnulada(partidaNuevaObj, decretoObj, targetParishId);
      const notaNueva = generarNotaAlMargenNuevaPartida(partidaAnuladaObj, decretoObj, nombreSacerdote, targetParishId);
      ...
      const newPartidaRaw = {
        ...newPartidaData,
        id: newPartidaId, parishId: targetParishId,
        book_number: partidaNuevaObj.libro, page_number: partidaNuevaObj.folio, entry_number: partidaNuevaObj.numero,
        status: 'seated', isSupplementary: true, creadoPorDecreto: true, hasDecree: true,
        correctionDecreeRef: decreeData.decreeNumber, conceptoAnulacionId: decreeData.conceptoAnulacionId,
        tipoNotaAlMargen: 'porCorreccion.nuevaPartida',
      ...
      // 3. Guardar en Supabase (Partidas anulada y nueva)
      try {
        await saveBaptismToSource(updatedOriginalRaw, targetParishId, updatedOriginalRaw.status);
        await saveBaptismToSource(newPartidaRaw, targetParishId, newPartidaRaw.status);
      } catch (e) { console.warn("Fallo menor al subir partidas a la nube:", e); }

      // 4. CÁPSULA DEL DECRETO (AQUÍ SE INYECTA LA OBSERVACIÓN PARA QUE LLEGUE AL IMPRESO)
      const decreeId = generateUUID();
      const decreeRecord = {
        id: decreeId,
        ...decreeData,
        ...
        numeroDecreto: decreeData.decreeNumber,
        fechaDecreto: decreeData.decreeDate,
        observaciones: decreeData.observaciones, // 🚀 Captura el texto del formulario
        originalPartidaId,
        newPartidaId: newPartidaId,
        targetParishId: targetParishId,
        targetParishName: decreeData.targetParishName,
        originalPartidaSummary: {
          ...updatedOriginalRaw,
          book: origLibro,
          page: origFolio,
      ...
      // 5. Enviar el Decreto a Supabase
      try {
        await supabase.from('decretos').insert([{ id: decreeId, parish_id: targetParishId, tipo: 'correccion', payload: decreeRecord }]);
      } catch (e) { }
  ...
  return (
    <AppDataContext.Provider value={{
        ...
        getMatrimonios, getPendingMatrimonios, saveMatrimonioToSource, seatMatrimonio, seatMultipleMatrimonios, getNextMatrimonioNumbers, getMatrimonioParameters, updateMatrimonioParameters,
        getConfirmations, getPendingConfirmations, saveConfirmationToSource, seatConfirmation, seatMultipleConfirmations, getNextConfirmationNumbers, getConfirmationParameters, updateConfirmationParameters,
        importConfirmations, addConfirmationsFromJSON,
        getDiocesis, addDiocesis, updateDiocesis, deleteDiocesis,
        getIglesiasList, addIglesia, updateIglesia, deleteIglesia,
